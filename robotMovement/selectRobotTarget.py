    ...
    if state == SEARCH_BALLS:
        ...
        if targetBall == None:
            if detectedObjects.get("whiteBalls") and len(detectedObjects["whiteBalls"]) > 0:
                ...
                # if avoidObstacle(robotPos, targetBall, detectedObjects["cross"], robotWidth=119) is not None:
                #     print("Obstacle in the way, navigating to intermediaty point")
                #     intermediaryPoint = avoidObstacle(robotPos, targetBall, detectedObjects["cross"], robotWidth=119)
                #     stateQueue.pop(0)
                #     stateQueue.append((TO_INTERMEDIARY, intermediaryPoint))
                #     stateQueue.append((SEARCH_BALLS, ""))
